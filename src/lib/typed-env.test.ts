import { ProjectEnvName } from '../generated/env-name';
import { anyEnv, typedEnv } from './typed-env';

test('boolean env', () => {
  const envBox = typedEnv<ProjectEnvName>('ENV_TEXT_BOOLEAN');
...
test('integer env', () => {
  const envBox = typedEnv<ProjectEnvName>('ENV_TEST_NUMBER');
  const rs = envBox.required().toString();
...
test('string env', () => {
  const envBox = typedEnv<ProjectEnvName>('ENV_TEST_STRING');
  const rs = envBox.required().toString();
...
test('json env', () => {
  const envBox = typedEnv<ProjectEnvName>('ENV_TEST_JSON');
  const rs = envBox.required().toString();
